...
class Experiment2:
    ...
    def human_oracle(self, sample_list) -> bool:
        ...
        return SBR_num >= Experiment2.SBR_threshold
    ...
    def run(self):
        ...
        target = int(real_pos_num * Experiment2.recall_threshold) + 1
        # 初始化采样循环
        while True:
            # 随机初始化采样方法
            if Experiment2.init_sample_method == "random":
                sample_list = self.encoder.random_sample(self.unlabeled_set, Experiment2.sample_number)
            # 风险策略初始化采样方法
            elif Experiment2.init_sample_method == "risk":
                sample_list = self.encoder.risk_sample(self.unlabeled_set, self.encoded_data_dict,
                                                       Experiment2.sample_number)
            else:
                return
        ...
        for i in range(Experiment2.learning_cycle):
            # 已经标记过的原始数据
            ...
            unlabeled_x_train, unlabeled_y_train = self.get_data_and_label(random.sample(
                self.unlabeled_set, max(len(labeled_pos_data_id), Experiment2.pnr_sample)))
            ...
            if len(labeled_pos_data_id) >= Experiment2.aggressive_threshold:
            ...
            prob = self.model.predict(unlabeled_x_predict)
            # 不确定采样
            if len(labeled_pos_data_id) < Experiment2.query_threshold:
                order = np.argsort(np.abs(np.array(prob) - 0.5))[:Experiment2.sample_number]
            # 确定性采样
            else:
                order = np.argsort(prob)[::-1][:Experiment2.sample_number]
